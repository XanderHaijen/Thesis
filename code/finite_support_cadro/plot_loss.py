...
def main():
    ...
    for i in range(len(theta)):
        results = ellipsoidal_cadro(data, data_test, tau, theta=theta[i], ellipse_alg='circ',
                                    scaling_factor_ellipse=scaling_factor_ellipse, theta_0=theta_0)
        loss[i] = results['train_loss']
        objective[i] = results['lambda'] * results['alpha'] + results['tau']

    # get theta_0, theta_r and theta_star
    results = ellipsoidal_cadro(data, data_test, tau, ellipse_alg='circ',
                                scaling_factor_ellipse=scaling_factor_ellipse, theta_0=theta_0, plot=True)
    ...
    plt.figure()
    plt.plot(theta, 5*loss, label="loss function")
